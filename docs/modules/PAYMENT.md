...
It also supports Creem as an alternative provider with the same interfaces and flows.

## Structure

### Core Payment Module
- `apps/web/src/payment/types.ts` - Type definitions for the payment module
- `apps/web/src/payment/index.ts` - Main payment interface and global provider instance
- `apps/web/src/payment/provider/stripe.ts` - Stripe payment provider implementation

### Server Actions
- `apps/web/src/actions/create-checkout-session.ts` - Server action for creating checkout sessions
- `apps/web/src/actions/create-customer-portal-session.ts` - Server action for creating customer portal sessions
- `apps/web/src/actions/check-payment-completion.ts` - Server action for checking payment completion status
- `apps/web/src/actions/create-credit-checkout-session.ts` - Server action for creating credit package checkout sessions
- `apps/web/src/actions/consume-credits.ts` - Server action for consuming user credits
- `apps/web/src/actions/get-credit-balance.ts` - Server action for getting user credit balance
- `apps/web/src/actions/get-credit-stats.ts` - Server action for getting credit statistics
- `apps/web/src/actions/get-credit-transactions.ts` - Server action for getting credit transaction history
- `apps/web/src/actions/get-current-plan.ts` - Server action for getting current user plan and subscription data

### API Routes
- `apps/web/src/app/api/webhooks/stripe/route.ts` - API route for Stripe webhook events

### Pages
- `apps/web/src/app/[locale]/(protected)/payment/page.tsx` - Payment processing page with status display
- `apps/web/src/app/[locale]/(protected)/settings/billing/page.tsx` - Account billing page to manage subscriptions
- `apps/web/src/app/[locale]/(protected)/settings/credits/page.tsx` - Credits management page
- `apps/web/src/app/[locale]/(marketing)/pricing/page.tsx` - Pricing page using the pricing table component

### Components

#### Payment Components
- `apps/web/src/components/payment/payment-card.tsx` - Payment status display component with polling

#### Pricing Components
- `apps/web/src/components/pricing/pricing-card.tsx` - Component to display a single pricing plan
- `apps/web/src/components/pricing/pricing-table.tsx` - Component to display all pricing plans
- `apps/web/src/components/pricing/create-checkout-button.tsx` - Button component to initiate checkout
- `apps/web/src/components/pricing/customer-portal-button.tsx` - Button component to access Stripe customer portal

#### Settings Components
- `apps/web/src/components/settings/billing/billing-card.tsx` - Billing management card component
- `apps/web/src/components/settings/credits/credit-packages.tsx` - Credit packages display component
- `apps/web/src/components/settings/credits/credit-checkout-button.tsx` - Credit package checkout button
- `apps/web/src/components/settings/credits/credit-detail-viewer.tsx` - Credit detail viewer component
- `apps/web/src/components/settings/credits/credit-transactions-table.tsx` - Credit transactions table component
- `apps/web/src/components/settings/credits/credit-transactions.tsx` - Credit transactions component
- `apps/web/src/components/settings/credits/credits-card.tsx` - Credits card component
- `apps/web/src/components/settings/credits/credits-page-client.tsx` - Credits page client component

### Hooks
- `apps/web/src/hooks/use-payment-completion.ts` - Hook for checking payment completion with polling
- `apps/web/src/hooks/use-payment.ts` - Hooks for payment-related data fetching (current plan, subscription status)
- `apps/web/src/hooks/use-credits.ts` - Hooks for credit-related operations (balance, stats, transactions, consumption)

## Select Provider via Env

Set which payment provider to use:

```
NEXT_PUBLIC_PAYMENT_PROVIDER=stripe   # or 'creem'
```
...
## Environment Variables (Creem)

```
# Core
CREEM_API_KEY=sk_live_or_test
CREEM_TEST_MODE=true

# Creem Product IDs mapped to our priceId fields when provider=creem
...
When `CREEM_TEST_MODE=true`, the provider uses Creem test environment (serverIdx=1). Set it to `false` for production.

## Payment Plans

Payment plans are defined in `apps/web/src/config/website.tsx`. Each plan can have multiple pricing options (monthly, yearly, one-time) with the following structure:

```typescript
// In apps/web/src/config/website.tsx
...
#### `apps/web/src/actions/create-checkout-session.ts`
```typescript
...
#### `apps/web/src/actions/create-customer-portal-session.ts`
```typescript
...
#### `apps/web/src/actions/check-payment-completion.ts`
```typescript
...
#### `apps/web/src/actions/consume-credits.ts`
```typescript
...
// Used in apps/web/src/app/[locale]/(protected)/payment/page.tsx
```
...
Stripe webhook events are handled via `apps/web/src/app/api/webhooks/stripe/route.ts`, which calls the `handleWebhookEvent` function from the payment module.

Creem webhook events are handled via `apps/web/src/app/api/webhooks/creem/route.ts`.
...
For Creem, the handler processes events:

- `checkout.completed` (one-time payments; also used for credit purchases if the productId matches a credit package)
- `subscription.paid` (new or renewal payments)
- `subscription.canceled`
- `subscription.expired`

Notes:
- In Creem mode, `priceId` in our configuration corresponds to Creem `productId`.
- Customer Portal is generated via Creem SDK `generateCustomerLinks`.
...
The main payment interface in `apps/web/src/payment/index.ts` provides these global functions:
