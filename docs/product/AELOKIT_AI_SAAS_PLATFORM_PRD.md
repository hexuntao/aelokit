...
从产品团队视角，AeloKit 提供一套可演进的 AI SaaS 架构。它将 assistant-ui 作为 AI 工作台 UI 层，将 Vercel AI SDK 作为 streaming 和 message protocol 层，将 app API routes 作为认证、策略和传输边界，将 app runtime wiring 作为 provider、model、agent、tool 和 policy 的接线层，将可复用 AI contracts 收敛到 AI infrastructure core，并在需要复杂编排时使用 Mastra 负责 agent、workflow、tool、memory、RAG 和 MCP runtime。
...
39. 作为开发者，我想只在需要复杂 agent/workflow/tool 编排时使用 Mastra，从而避免简单聊天路径过度复杂。
...
- Agent、workflow、tool、memory、RAG 和 MCP 的复杂编排由 Mastra 承担；简单 chat 不强制经过 Mastra。
...
- Memory layer 与 Knowledge layer 必须分开。Memory 是系统可长期复用的用户、Agent、项目或 thread 行为上下文；Knowledge 是可追溯来源、可引用的资料检索内容。
...
- Memory 模块应测试写入确认、删除策略、thread summary 更新和不同 memory scope 的隔离。
- Knowledge 模块应测试 document/chunk/source/citation 的可追溯性，以及 storage 关系不丢失。
