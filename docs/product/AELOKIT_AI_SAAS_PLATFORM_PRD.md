...
50. 作为团队维护者，我想用 PRD 统一产品目标，从而让后续工程任务都能追溯到用户价值和商业闭环。

## Implementation Decisions

- AeloKit 的产品定位是 AI-native SaaS engineering foundation，而不是单一 chatbot、模型网关或纯 UI starter。
- 历史 `v0.1 ~ v0.8` 只保留为高层演进叙述；当前工程任务以用户 prompt + 本 PRD + `AGENTS.md` 为准，不再依赖已删除 roadmap。
...
- 本 PRD 是当前产品北极星；具体工程任务由用户 prompt 和 `AGENTS.md` 的工程边界约束。

## Further Notes

- 如果未来需要工程路线图，必须由当前 PRD + 用户任务重新确认；不要从已删除的历史 roadmap 自动恢复建设顺序。
...
- 后续每个工程阶段都应该能回答两个问题：它服务 PRD 中的哪个产品闭环，以及用户当前任务是否明确打开了该建设范围。
