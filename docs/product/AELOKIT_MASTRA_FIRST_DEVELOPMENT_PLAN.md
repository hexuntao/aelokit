...
- Phase 1 已达到 code-level complete：主链路是 Mastra-agent-shaped runner，但仍不是
  Mastra-native streaming。
- Phase 2 已达到 code-level complete：memory / knowledge context 已接入，但 runtime
  DB/vector/provider smoke 仍未验证。
- Phase 3 在 tool gating 修复前只能算 partial；修复后可视为 code-level accepted。
...
`/api/ai/chat` 已调用 `runMastraChat()`，但 runner 内部仍使用 Vercel AI SDK
`streamText()` 生成 AI SDK-compatible UI message stream；这属于
Mastra-agent-shaped runner，不是 Mastra-native streaming。runtime smoke 未验证。
...
当前状态：code-level complete。memory / knowledge context 已接入当前 agent path；
runtime DB/vector/provider smoke NOT VERIFIED。
...
当前状态：在 tool gating 修复前为 partial。本任务修复 knowledge tool gating 后，
可标记为 code-level accepted；runtime tool smoke 仍未验证。
