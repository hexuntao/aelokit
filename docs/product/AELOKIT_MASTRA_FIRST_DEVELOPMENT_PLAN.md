...
当前代码已经进入 Mastra-first Agent Core 的第一层闭环，并已切到 Mastra Agent
streaming bridge；runtime smoke 仍未验证：

- `apps/web/package.json` 已有 `@mastra/core`、`@mastra/memory`、
  `@mastra/pg`、`@mastra/rag` 和 `@mastra/ai-sdk`。
...
- `runMastraChat()` 当前使用 Mastra `Agent.stream()`，再通过
  `toAISdkStream(..., { from: 'agent', version: 'v6' })` 输出 AI SDK v6-compatible
  UI message stream；它不是旧的 Vercel AI SDK `streamText()` wrapper。
- `apps/web/src/ai/memory/*` 与 `apps/web/src/ai/memory-service.ts` 已使用
  Mastra memory 相关能力处理 confirmed memory。本次 hardening 后，confirmed
  user memory 作为 system context 注入，不再 prepend 到当前 conversation
  `inputMessages`；Mastra conversation auto-persistence 暂不启用，以避免
  assistant-ui full-history payload、AeloKit app DB persistence 和 Mastra message
  ordering 产生不受控双写或重复上下文。
- `apps/web/src/ai/knowledge/*` 已使用 Mastra RAG/vector 相关能力处理 chunking
  和 vector retrieval。
- `apps/web/src/ai/tools/*` 当前只允许 app-local registry 产物进入 Agent。
  Client-provided/frontend tools 不会经 `frontendTools()` 直接合并进 Agent；未注册
  client tool 会生成 deny permission decision，并进入 audit-safe metadata。
...
- `apps/web/src/ai/mastra/index.ts` 当前导出 Mastra storage、memory、context 和
  runner surface；它仍是 app-local runtime wiring，不是 `packages/ai` contract
  surface。

当前代码事实不是“下一步推荐 Phase 1”，而是以下状态：

- Phase 1 已达到 code-level complete：主链路使用 Mastra `Agent.stream()` 和
  `@mastra/ai-sdk` 的 AI SDK v6 stream bridge；authenticated runtime smoke 仍未验证。
- Phase 2 当前为 code-level hardened：memory / knowledge context 已接入，confirmed
  memory 避免重复 message context；runtime DB/vector/provider smoke 仍 NOT VERIFIED。
- Phase 3 当前为 code-level hardened：read-only `knowledge.inspect` 继续作为第一个
  registry tool，未注册 client tool 被拒绝/忽略且 permission decision 可审计；runtime
  tool smoke 仍 NOT VERIFIED。
- Phase 4 是 AeloKit app-local auditable workflow foundation，不是 Mastra-native
  workflow primitive 完整落地。
- Phase 5 仍是 partial：admin audit 与 credits controls 已存在，但 production
  charging 仍阻塞于真实 reserve/hold 语义。
- Phase 6 已遵守 split gate：未创建 future apps/packages，但这不是 feature-complete
  split phase。
...
目标：把主 chat generation path 保持在 Mastra Agent path，同时保持现有 route 和
UI contract 稳定。

当前状态：code-level complete。app-local Mastra chat runner 已存在，
`/api/ai/chat` 已调用 `runMastraChat()`；runner 内部使用 Mastra `Agent.stream()`
并通过 `@mastra/ai-sdk` 的 `toAISdkStream(..., { version: 'v6' })` 返回
AI SDK-compatible UI message stream。runtime smoke 未验证。
...
- `@mastra/ai-sdk` 已安装并用于 Mastra-to-AI-SDK v6 stream bridge。
- 没有 authenticated browser/provider/DB evidence 前，Phase 1 只能标记
  code-level complete，不能标记 runtime PASS。
...
当前状态：code-level hardened。memory / knowledge context 已接入当前 agent path；
confirmed user memory 只作为 system context 注入，不再 prepend 到
`inputMessages`；runtime DB/vector/provider smoke NOT VERIFIED。
...
- Mastra memory/storage 当前用于 confirmed user memory；Mastra conversation
  auto-persistence 暂不启用，直到 client/server 能安全发送 only-latest-message 并
  明确 app DB 与 Mastra message history 的职责边界。
...
当前状态：code-level hardened。`knowledge.inspect` 是第一个 read-only registry
tool；未注册 client-provided/frontend tools 被拒绝或忽略，并记录 audit-safe
permission decision。runtime tool smoke 仍未验证。
...
- 定义 app-local Mastra tool registry，映射到 `packages/ai` 的 tool / permission
  contracts。
- 从 read-only、低风险工具开始。
- 不允许 request body raw `tools` 通过 `frontendTools()` 直接合并进 Agent。
...
当前状态：AeloKit app-local auditable workflow foundation 已建立，但它不是
Mastra-native workflow primitive 的 feature-complete 落地。
...
当前状态：partial。admin audit + credits controls 已存在；本任务修复 admin usage
audit cost filter。production charging 仍阻塞于真实 reserve/hold semantics，
不能据此宣称可安全上线扣费。
...
当前状态：split gate 已遵守；未创建 future apps/packages。但这不代表已进入
feature-complete split phase。
