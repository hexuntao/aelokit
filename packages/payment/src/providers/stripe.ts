...
import type { Payment as PaymentRecord } from '@repo/db/types';
...
export interface StripeWebhookCallbacks {
  ...
  paymentRecordRetryAttempts: number;
  paymentRecordRetryDelay: number;
}
 ...
 *
 * docs:
 * https://example.com/docs/payment
 */
export class StripeProvider implements PaymentProvider {
  private stripe: Stripe;
  private webhookSecret: string;
  private callbacks: StripeWebhookCallbacks;

  /**
   * Initialize Stripe provider with API key
   */
  constructor(callbacks: StripeWebhookCallbacks) {
  ...
  private async findPaymentRecordWithRetry(
    ...
    for (
      let attempt = 1;
      attempt <= this.callbacks.paymentRecordRetryAttempts;
      attempt++
    ) {
      ...
      if (attempt < this.callbacks.paymentRecordRetryAttempts) {
        console.log(
          `Payment record not found, retry in ${this.callbacks.paymentRecordRetryDelay}ms`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, this.callbacks.paymentRecordRetryDelay)
        );
      }
    }
  ...
  private async processSubscriptionPurchase(
    ...
    if (websiteConfig.credits?.enableCredits) {
      await this.callbacks.addSubscriptionCredits(userId, priceId);
  ...
  private async processLifetimePlanPurchase(
    ...
    if (websiteConfig.credits?.enableCredits) {
      await this.callbacks.addLifetimeMonthlyCredits(
    ...
    // Send notification
    const amount = invoice.amount_paid ? invoice.amount_paid / 100 : 0;
    await this.callbacks.sendPaymentNotification({
