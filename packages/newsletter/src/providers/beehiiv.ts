...
export class BeehiivNewsletterProvider implements NewsletterProvider {
  ...
  constructor() {
    if (!serverEnv.BEEHIIV_API_KEY) {
      throw new Error('BEEHIIV_API_KEY environment variable is not set.');
    }

    if (!serverEnv.BEEHIIV_PUBLICATION_ID) {
      throw new Error(
        'BEEHIIV_PUBLICATION_ID environment variable is not set.'
      );
    }

    this.client = new BeehiivClient({ token: serverEnv.BEEHIIV_API_KEY });
    this.publicationId = serverEnv.BEEHIIV_PUBLICATION_ID;
  }
